...
namespace CXD.Tests
{
    public abstract class CXDTestBase : AbpIntegratedTestBase<CXDTestModule>
    {
        ...
        protected void UsingDbContext(Action<CXDDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CXDDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CXDDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CXDDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CXDDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CXDDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CXDDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CXDDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CXDDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CXDDbContext, Task<T>> func)
        {
            T result;
